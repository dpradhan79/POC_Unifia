...
public class BasicLogon_GWGen {
	...
	public void edge(String browserP, String URL, String AdminDB) throws InterruptedException, StopConditionException, URISyntaxException, IOException {
	 ...
	 String FileA = "/yED_graphs/BasicLogin.graphml";
	 ...
	 try{
		  // Create file 
		  FileWriter fstream = new FileWriter("BasicLogon_Edge_Summary.txt");
	...
	public void vertex()throws InterruptedException, StopConditionException, URISyntaxException {
		 Model model = new Model();
		 // Get the model from resources
		 	/* URL url = ModelHandler.class.getResource("/yED_graphs/BasicLogin.graphml");
		...
		/* modelhandler.add("Sprint1_Edge", new AdminConsole_GWAPI(file, true, new RandomPathGenerator(new VertexCoverage (1)), false));
		 // Start executing the test
		 modelhandler.execute("Sprint1_Edge");
		 ...
		 try{
			  // Create file 
			  FileWriter fstream = new FileWriter("BasicLogon_Vertex_Summary.txt");
			  BufferedWriter out = new BufferedWriter(fstream);
			  out.write(actualResult);
			  //Close the output stream
			  out.close();
	...
	@Test(dataProvider="BrowserandURL",dataProviderClass=Unifia_Admin_Selenium.class)
	public void Duration(String browserP, String URL, String AdminDB) throws InterruptedException, StopConditionException, URISyntaxException, IOException {
			//select the Driver type Grid or local
			if (Unifia_Admin_Selenium.parallelExecutionType && Unifia_Admin_Selenium.driverType.equalsIgnoreCase("LOCAL")){
				System.out.println("If execution type is parallel i.e., 'parallelExecutionType = true' then driverType should be always 'GRID'");
				System.exit(1);
		 ...
		 try{
			  // Create file 
			  FileWriter fstream = new FileWriter("BasicLogon_TimeDuration_Summary.txt");
			  BufferedWriter out = new BufferedWriter(fstream);
			  out.write(actualResult);
			  //Close the output stream
			  out.close();
