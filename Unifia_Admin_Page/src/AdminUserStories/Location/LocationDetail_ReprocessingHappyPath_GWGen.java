...
public class LocationDetail_ReprocessingHappyPath_GWGen {
	...
	Unifia_Admin_Selenium.DriverSelection(browserP,URL,AdminDB);		
	 ...
	 String FileB = "/yED_graphs/LocationDetails_Reprocessing_happyPath.graphml";
	 ...
	 PathGenerator pathGenerator = new RandomPath(new EdgeCoverage(100));
	  ...
	  Unifia_Admin_Selenium.FileName="LocationReprocessingHappyPath_TestSummary_";
	 ...
	 try{
		  // Create file 
		  FileWriter fstream = new FileWriter("LocationReprocessingHappy_Edge_Summary.txt");
	...
	public void vertex()throws InterruptedException, StopConditionException, URISyntaxException {
		 ...
		 // Get the model from resources
		 	URL url_A = ModelHandler.class.getResource("/yED_graphs/BasicLogin_Location.graphml"); 
		 	URL url_B = ModelHandler.class.getResource("/yED_graphs/LocationDetails_Reprocessing_happyPath.graphml");
		 ...
		 modelhandler.add("LocationDetail", new LocationDetails_GWAPI(file_B, true, new RandomPathGenerator(new VertexCoverage (1)), false));
	...
	try{
		  // Create file 
		  FileWriter fstream = new FileWriter("LocationReprocessingHappy_Vertex_Summary.txt");
		  BufferedWriter out = new BufferedWriter(fstream);
		  out.write(actualResult);
		  //Close the output stream
		  out.close();
