...
public class ScopeSafetyDetailsGWGen {
	public TestFrameWork.UnifiaAdminLandingPage.LandingPage_Actions LP_A;
	public AdminUserStories.ScopeSafety.ScopeSafetyDetail_GWAPI AB;
	...
	public void edge(String browserP, String URL,String AdminDB) throws InterruptedException, StopConditionException, URISyntaxException, IOException {
	 ...
	 // Stop condition: Edge coverage 100%, we want to walk every edge in the model.
	 	 String FileA = "/yED_graphs/BasicLogin_ScopeSafety.graphml";
		 String FileB = "/yED_graphs/ScopeSafetyDetails.graphml";
		 ...
		 PathGenerator pathGenerator = new RandomPath(new EdgeCoverage(100));
		  ...
		  Result result =  new TestBuilder()
			 ...
			 .addModel(FileB, new ScopeSafetyDetail_GWAPI().setPathGenerator(new RandomPath((StopCondition) new EdgeCoverage(100))))
			 .execute();
	 ...
	 try{
		  // Create file 
		  FileWriter fstream = new FileWriter("ScopeSafety_Edge_Summary.txt");
	...
	public void vertex()throws InterruptedException, StopConditionException, URISyntaxException {
		 ...
		 // Get the model from resources
		 URL url_A = ModelHandler.class.getResource("/yED_graphs/BasicLogin_ScopeSafety.graphml"); 
		 URL url_B = ModelHandler.class.getResource("/yED_graphs/ScopeSafetyDetails.graphml");
		 ...
		 modelhandler.add("ScopeSafety", new ScopeSafetyDetail_GWAPI(file_B, true, new RandomPathGenerator(new VertexCoverage (1)), false));
		 ...
		 try{
			  // Create file 
			  FileWriter fstream = new FileWriter("ScopeSafety_Vertex_Summary.txt");
			  BufferedWriter out = new BufferedWriter(fstream);
			  out.write(actualResult);
			  //Close the output stream
			  out.close();
