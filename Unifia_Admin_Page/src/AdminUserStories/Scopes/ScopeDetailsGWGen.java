...
import AdminUserStories.Scanner.ScannerDetail_GWAPI;
import AdminUserStories.Scanner.ScannerDetailsGWGen.TestContext;
import AdminUserStories.Scopes.ScopeDetails_GWAPI;
...
public class ScopeDetailsGWGen {
	public TestFrameWork.UnifiaAdminLandingPage.LandingPage_Actions LP_A;
	public AdminUserStories.Scopes.ScopeDetails_GWAPI AB;
	...
	public void edge(String browserP, String URL, String AdminDB) throws InterruptedException, StopConditionException, URISyntaxException, IOException {
	 ...
	 String FileA = "/yED_graphs/BasicLogin_Scope.graphml";
	 String FileB = "/yED_graphs/ScopeDetails.graphml";
	 ...
	 PathGenerator pathGenerator = new RandomPath(new EdgeCoverage(100));
	  ...
	  Result result =  new TestBuilder()
		 ...
		 .addModel(FileB, new ScopeDetails_GWAPI().setPathGenerator(new RandomPath((StopCondition) new EdgeCoverage(100))))
		 .execute();
	 ...
	 try{
		  // Create file 
		  FileWriter fstream = new FileWriter("Scope_Edge_Summary.txt");
	...
	public void vertex()throws InterruptedException, StopConditionException, URISyntaxException {
		 ...
		 // Get the model from resources
		 	URL url_A = ModelHandler.class.getResource("/yED_graphs/BasicLogin_Scope.graphml"); 
		 	URL url_B = ModelHandler.class.getResource("/yED_graphs/ScopeDetails.graphml");
		 ...
		 modelhandler.add("ScopeDetail", new ScopeDetails_GWAPI(file_B, true, new RandomPathGenerator(new VertexCoverage (1)), false));
		 ...
		 try{
			  // Create file 
			  FileWriter fstream = new FileWriter("Scope_Vertex_Summary.txt");
			  BufferedWriter out = new BufferedWriter(fstream);
			  out.write(actualResult);
			  //Close the output stream
			  out.close();
