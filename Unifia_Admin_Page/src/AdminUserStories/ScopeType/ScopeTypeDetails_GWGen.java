...
public class ScopeTypeDetails_GWGen {
	public TestFrameWork.UnifiaAdminLandingPage.LandingPage_Actions LP_A;
	public AdminUserStories.ScopeType.ScopeTypeDetails_GWAPI AB;
	...
	// modelhandler.add("BasicLogin", new AdminConsole_GWAPI(file_A,true, new RandomPathGenerator(new EdgeCoverage(1)),false));
	 String FileA = "/yED_graphs/BasicLogin_ScopeType.graphml";
	 String FileB = "/yED_graphs/ScopeTypeDetail.graphml";
	 ...
	 PathGenerator pathGenerator = new RandomPath(new EdgeCoverage(100));
	  ...
	  Result result =  new TestBuilder()
		 ...
		 .addModel(FileB, new ScopeTypeDetails_GWAPI().setPathGenerator(new RandomPath((StopCondition) new EdgeCoverage(100))))
		 .execute();
	 ...
	 try{
		  // Create file 
		  FileWriter fstream = new FileWriter("Scope_Model_Edge Summary.txt");
...
/*@Test
	public void vertex()throws InterruptedException, StopConditionException, URISyntaxException {
		 ...
		 // Get the model from resources
		 	URL url_A = ModelHandler.class.getResource("/yED_graphs/BasicLogin_ScopeType.graphml"); 
		 	URL url_B = ModelHandler.class.getResource("/yED_graphs/ScopeTypeDetail.graphml");
		 ...
		 modelhandler.add("ScopeTypeDetails", new ScopeTypeDetails_GWAPI(file_B, true, new RandomPathGenerator(new VertexCoverage (1)), false));
...
try{
	  // Create file 
	  FileWriter fstream = new FileWriter("Scope_Model_Vertex_Summary.txt");
	  BufferedWriter out = new BufferedWriter(fstream);
	  out.write(actualResult);
	  //Close the output stream
	  out.close();
