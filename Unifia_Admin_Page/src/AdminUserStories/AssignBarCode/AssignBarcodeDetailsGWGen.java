...
import AdminUserStories.AssignBarCode.AssignBarcodeDetail_GWAPI;
...
public class AssignBarcodeDetailsGWGen {
	public TestFrameWork.UnifiaAdminLandingPage.LandingPage_Actions LP_A;
	public AdminUserStories.AssignBarCode.AssignBarcodeDetail_GWAPI AB;
	...
	public void edge(String browserP, String URL,String AdminDB) throws InterruptedException, StopConditionException, URISyntaxException, IOException {
	 ...
	 String FileA = "/yED_graphs/BasicLogin_AssignBarCode.graphml";
	 String FileB = "/yED_graphs/AssignBarcodeDetails.graphml";
	 ...
	 PathGenerator pathGenerator = new RandomPath(new EdgeCoverage(100));
	  ...
	  Result result =  new TestBuilder()
		 ...
		 .addModel(FileB, new AssignBarcodeDetail_GWAPI().setPathGenerator(new RandomPath((StopCondition) new EdgeCoverage(100))))
		 .execute();
	 ...
	 try{
		  // Create file 
		  FileWriter fstream = new FileWriter("AssignBarcode_Edge_Summary.txt");
	...
	public void vertex()throws InterruptedException, StopConditionException, URISyntaxException {
		 Model model = new Model();
		 // Get the model from resources
		/* URL url_A = ModelHandler.class.getResource("/yED_graphs/BasicLogin_AssignBarCode.graphml"); 
		 URL url_B = ModelHandler.class.getResource("/yED_graphs/AssignBarcodeDetails.graphml");
		 ...
		 PathGenerator pathGenerator = new RandomPath(new VertexCoverage(100));
		  ...
		  Result result =  new TestBuilder()
			 ...
			 .addModel(FileB, new AssignBarcodeDetail_GWAPI().setPathGenerator(new RandomPath((StopCondition) new VertexCoverage(100))))
			 .execute();
		 
		 /*modelhandler.add("BasicLogin", new AdminConsole_GWAPI(file_A,true, new RandomPathGenerator(new VertexCoverage(1)),false));
		 modelhandler.add("AssignBarcode", new AssignBarcodeDetail_GWAPI(file_B, true, new RandomPathGenerator(new VertexCoverage (1)), false));
		...
		// Assert.assertTrue(modelhandler.isAllModelsDone(), "Not all models are done");
		 ...
		 try{
			  // Create file 
			  FileWriter fstream = new FileWriter("AssignBarcode_Vertex_Summary.txt");
			  BufferedWriter out = new BufferedWriter(fstream);
			  out.write(actualResult);
			  //Close the output stream
			  out.close();
