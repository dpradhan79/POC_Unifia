...
import AdminUserStories.Facility.FacilityDetail_GWAPI;
...
public class FacilityGWGen {
	
	public TestFrameWork.UnifiaAdminLandingPage.LandingPage_Actions LP_A;
	public AdminUserStories.Facility.FacilityDetail_GWAPI AB;
    ...
    public void edge(String browserP, String URL,String AdminDB) throws InterruptedException, StopConditionException, URISyntaxException, IOException {
	 ...
	 String FileA = "/yED_graphs/BasicLogin_Facility.graphml";
	 String FileB = "/yED_graphs/FacilityDetail.graphml";
	 ...
	 PathGenerator pathGenerator = new RandomPath(new EdgeCoverage(100));
	  ...
	  Result result =  new TestBuilder()
		 ...
		 .addModel(FileB, new FacilityDetail_GWAPI().setPathGenerator(new RandomPath((StopCondition) new EdgeCoverage(100))))
		 .execute();
	 ...
	 try{
		  // Create file 
		  FileWriter fstream = new FileWriter("Facility_Edge_Summary.txt");
	...
	public void vertex()throws InterruptedException, StopConditionException, URISyntaxException {
		 ...
		 // Get the model from resources
		 	...
		 	URL url_A = ModelHandler.class.getResource("/yED_graphs/BasicLogin_Facility.graphml");
		 	URL url_C = ModelHandler.class.getResource("/yED_graphs/FacilityDetail.graphml");
		 ...
		 //modelhandler.add("Facility", new Facility_GWAPI(file_B, true, new RandomPathGenerator(new VertexCoverage (1)), false));
		 modelhandler.add("FacilityDetail", new FacilityDetail_GWAPI(file_C, true, new RandomPathGenerator(new VertexCoverage (1)), false));
		 ...
		 try{
			  // Create file 
			  FileWriter fstream = new FileWriter("Facility_Vertex_Summary.txt");
			  BufferedWriter out = new BufferedWriter(fstream);
			  out.write(actualResult);
			  //Close the output stream
			  out.close();
