...
import AdminUserStories.ScopeSafety.ScopeSafetyDetail_GWAPI;
import AdminUserStories.ScopeSafety.ScopeSafetyDetailsGWGen.TestContext;
import AdminUserStories.Staff.StaffDetailsGWAPI;
...
public class StaffDetailsGWGen {
	public TestFrameWork.UnifiaAdminLandingPage.LandingPage_Actions LP_A;
	public AdminUserStories.Staff.StaffDetailsGWAPI AB;
	...
	public void edge(String browserP, String URL,String AdminDB) throws InterruptedException, StopConditionException, URISyntaxException, IOException {
	 ...
	 String FileA = "/yED_graphs/BasicLogin_Staff.graphml";
	 String FileB = "/yED_graphs/StaffDetailsNew.graphml";
	 ...
	 PathGenerator pathGenerator = new RandomPath(new EdgeCoverage(100));
	  ...
	  Result result =  new TestBuilder()
		 ...
		 .addModel(FileB, new StaffDetailsGWAPI().setPathGenerator(new RandomPath((StopCondition) new EdgeCoverage(100))))
		 .execute();
	 ...
	 try{
		  // Create file 
		  FileWriter fstream = new FileWriter("Staff_Edge_Summary.txt");
	...
	public void vertex()throws InterruptedException, StopConditionException, URISyntaxException {
		 ...
		 // Get the model from resources
		 	URL url_A = ModelHandler.class.getResource("/yED_graphs/BasicLogin_Staff.graphml"); 
		 	URL url_B = ModelHandler.class.getResource("/yED_graphs/StaffDetailsNew.graphml");
		 ...
		 modelhandler.add("StaffDetail", new StaffDetailsGWAPI(file_B, true, new RandomPathGenerator(new VertexCoverage (1)), false));
		 ...
		 try{
			  // Create file 
			  FileWriter fstream = new FileWriter("Staff_Vertex_Summary.txt");
			  BufferedWriter out = new BufferedWriter(fstream);
			  out.write(actualResult);
			  //Close the output stream
			  out.close();
