...
public class ExamManagement_Historical {	
	/**
	...
	public void Test(String browserP, String URL, String AdminDB) throws InterruptedException, StopConditionException, URISyntaxException, IOException, SQLException, Exception {
		...
		UAS.XMLFileName="Exam_Management_Historical_";
		...
		for(int i=0;i<23;i++){
			...
			IP_A.Click_EM();
			//EM_A.Clear_Filter();
			EM_A.DateFilter(ExamDate,ExamDate);
			EM_A.ApplyFilter();
			...
			if(i==0){//verify a user can click into workflow details and hit cancel and the user is taken back to EM with the filters still applied. 
			...
			}else if(i==1){//verify a user can click into workflow details, make a change to Physician and hit save and the user is taken back to EM with the filters still applied. 
			    IP_A.Click_Details(RefNo);
    	...
    	try{
			...
			stmt="Select IH.ReceivedDateTime from ItemHistory IH Join Location l on IH.locationID_FK=l.locationID_PK "
					+ "where IH.ScanItemTypeID_FK in (3) and IH.CycleEventID_FK in (4) and Facilityid_fk=1 and "
