...
public class RegressionSoiledRoom_GWGen {
	...
	public ScannerMessages.SoiledRoom.RegressionSoiledRoom_GWAPI AB;
	...
	public void edge(String browserP, String URL, String AdminDB) throws InterruptedException, StopConditionException, URISyntaxException, IOException, SQLException, AWTException {
		...
		//TDF.insertKEMasterData(Unifia_Admin_Selenium.KE_Url,Unifia_Admin_Selenium.KE_UserName,Unifia_Admin_Selenium.KE_Pwd); //Commented By Debasish
		TestDataFunc.insertMasterData(Unifia_Admin_Selenium.url, Unifia_Admin_Selenium.user, Unifia_Admin_Selenium.pass,KE, Bioburden, Culture);
    	GeneralFunc.RestartIISServices(Unifia_Admin_Selenium.Env, Unifia_Admin_Selenium.userName, Unifia_Admin_Selenium.IISPass);
		   
    	Login_Actions.Launch_Unifia(Unifia_Admin_Selenium.Admin_URL);
		/*User_Actions.selectUserRoleNLogin(browserP, URL, Unifia_Admin_Selenium.roleMgr, Unifia_Admin_Selenium.userQV1, Unifia_Admin_Selenium.userQV1Pwd);
		Unifia_Admin_Selenium.driverSelectionSecond(browserP,URL,AdminDB);
		Login_Actions.Launch_UnifiaSecond(Unifia_Admin_Selenium.Emulator_URL);*/
	 ...
	 /*PathGenerator pathGenerator = new RandomPath(new EdgeCoverage(100));
	  ...
	  Result result =  new TestBuilder()
		 .addModel(FileA, new RegressionSoiledRoom_GWAPI().setPathGenerator(new RandomPath((StopCondition) new EdgeCoverage(100))))
		 .execute();
	 // Print the statistics from graphwalker
	 GetIHValues.Close_Exec_Log(Unifia_Admin_Selenium.XMLFileName, "Test Completed", Unifia_Admin_Selenium.TestCaseNumber);
	 ...
	 try{
		  // Create file 
		  FileWriter fstream = new FileWriter("RegressionSoiledRoom_Edge_Summary.txt");
	...
	public void vertex(String browserP, String URL,String AdminDB) throws InterruptedException, StopConditionException, URISyntaxException, IOException {
		...
		Model model = new Model();
		 ...
		 PathGenerator pathGenerator = new RandomPath(new VertexCoverage(100));
		  ...
		  Result result =  new TestBuilder()
			 .addModel(FileA, new RegressionSoiledRoom_GWAPI().setPathGenerator(new RandomPath((StopCondition) new VertexCoverage(100))))
			 .execute();
		 // Print the statistics from graphwalker
		 ...
		 try{
			  // Create file 
			  FileWriter fstream = new FileWriter("RegressionSoiledRoom_Vertex_Summary.txt");
			  BufferedWriter out = new BufferedWriter(fstream);
			  out.write(actualResult);
			  //Close the output stream
			  out.close();
